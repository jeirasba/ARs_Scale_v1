...
File  =  Dataset("./ERA5_ARs2019_Case_Study.nc")  #Data from ERA5 over the region of interest. This data contains 5 days. 
...
def Main_Analyzer(Array):
    ...
    while t < Array.shape[0]:
        if Array[t] < l1:
            ...
            go = 1
            t = t+1
            while go == 1 and t < Array.shape[0]:
                if Array[t] >= l1 :
                    event_IVT = np.append(event_IVT,Array[t])
                    event_t = np.append(event_t,t)
                    t = t+1
...
np.save("Output_Detections_Categories",OUTPUT)
